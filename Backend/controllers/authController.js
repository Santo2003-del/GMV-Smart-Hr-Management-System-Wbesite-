...
const superAdminLogin = async (req, res) => {
  try {
    ...
    const cleanEnvEmail = String(envEmail).trim();
    const cleanEnvPass = String(envPass).replace(/\r$/, ''); // Strip CR if present

    if (cleanEmail !== cleanEnvEmail || cleanPass !== cleanEnvPass) {
    ...
    res.json({
      user: { role: 'SuperAdmin', email: cleanEnvEmail, name: 'System Root' },
      token
    });
  } catch (err) {
