...
const isValidMobile = (m) => MOBILE_REGEX.test(String(m || '').trim());
...
const getCompanyProfile = async (req, res) => {
  try {
    ...
    const [
      ...
      // Fetch all active employees to calculate total salary
      User.find({ companyId, role: 'Employee', status: 'Active', isDeleted: { $ne: true } }).select('basicSalary salary'),

      // Fetch recent custom expenses for UI list
      Expense.find({ companyId }).sort({ date: -1 }).limit(20).lean(),

      // Aggregate Total Expenses for Current Month
      Expense.aggregate([
        {
          $match: {
            companyId: typeof companyId === 'string' ? new require('mongoose').Types.ObjectId(companyId) : companyId,
    ...
    // Calculate total employee salary expenditure (Company-wide monthly)
    const totalSalaryExpense = activeEmployees.reduce((sum, emp) => {
      return sum + (Number(emp.basicSalary || emp.salary) || 0);
    }, 0);

    // Use aggregated result for accuracy
    const totalCustomExpense = monthlyExpenseAgg[0]?.total || 0;

    res.status(200).json({
      company,
      employees,
      hrs,
      customExpenses: recentExpenses,
      stats: {
        totalEmployees,
        totalHRs,
        totalSalaryExpense,
        totalCustomExpense,
        overallExpenditure: totalSalaryExpense + totalCustomExpense
      }
    });
  } catch (error) {
...
const requestHrLimit = async (req, res) => {
  try {
    ...
    // 1. Create a Support Ticket for Super Admin
    ...
    await Ticket.create({
      ...
      email: company.email,
      issueType: 'HR_LIMIT_INCREASE',
      description: `Request for HR limit increase from ${company.name}. Current limit: ${company.maxHrAdmins || 0}`,
      status: 'Open'
    });

    // 2. Send email to Super Admin (if SMTP configured)
    const adminEmail = process.env.SUPER_ADMIN_EMAIL || 'support@smarthrms.com';
    await sendEmail({
      email: adminEmail,
      subject: `HR Limit Increase Request: ${company.name}`,
      message: `Company ${company.name} has requested an HR limit increase.\n\nCompany Email: ${company.email}\nCurrent Limit: ${company.maxHrAdmins || 0}\n\nPlease review this on your Super Admin panel.`
    });

    // 3. Mark the company request as pending
    await Company.findByIdAndUpdate(companyId, { hrLimitRequest: 'Pending' });

    res.json({ message: "Request sent to Super Admin. Ticket created and Email sent ✅" });
  } catch (e) {
...
const updateCompanyUser = async (req, res) => {
  try {
    ...
    if (email) {
      ...
      userToCheck.email = cleanEmail;
    }
    if (mobile) {
      if (!isValidMobile(mobile)) {
        return res.status(400).json({ message: 'Mobile number must be exactly 10 digits' });
      }
      userToCheck.mobile = String(mobile).trim();
    }
