...
const deleteEmployee = async (req, res) => {
  try {
    ...
    // ✅ Soft delete employee (keep attendance/leave data for history)
    try {
      user.isDeleted = true;
      ...
      // We still delete onboarding assignments as they are process-specific
      await OnboardingAssignment.deleteMany({ userId: user._id });
    } catch (e) {
      console.error('deleteEmployee soft-delete failed:', e?.message);
    }
  } catch (err) {
