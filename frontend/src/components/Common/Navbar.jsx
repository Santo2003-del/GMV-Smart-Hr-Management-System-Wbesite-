...
const Navbar = () => {
  ...
  // Desktop nav includes Partner With Us as a link
  const publicLinks = [
    ...PUBLIC_NAV,
  ...
  // Mobile nav - Partner With Us moves to the button section
  ...
  const currentLinksMobile = showRoleNav ? roleLinks : publicLinksMobile;
  ...
  return (
    <nav className="navbar">
      <div className={`nav-container ${scrolled ? "scrolled" : ""}`}>
        ...
        {/* MOBILE MENU */}
        <ul className={`menu mobile-menu ${menuOpen ? "open" : ""}`}>
          {currentLinksMobile.map((link, idx) => (
            <li key={idx}>
              <Link
                to={link.to}
                className={isActive(link.to)}
      ...
      <style>{`
        ...
        @media (max-width: 1024px) {
          ...
          .mobile-btn {
            width: 100%; padding: 12px; border-radius: 10px; font-weight: 700; text-align: center;
            border: none; text-decoration: none; cursor: pointer; font-size: 0.9rem;
            transition: all 0.2s ease;
            display: flex;
            align-items: center;
            justify-content: center;
          ...
          .mobile-btn.partner { background: transparent; color: #00ff9d; border: 1px solid rgba(0, 255, 157, 0.3); }
